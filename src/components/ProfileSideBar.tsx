...
export const ProfileSideBar = () => {
  ...
  return (
    <>
      {photoid ? (
        <Photo src={photourl(photoid)} />
      ) : (
        <Photo src={photourl("0684456b-aa2b-4631-86f7-93ceaf33303c_ziuody")} />
      )}
      {setting ? (
        <>
          <HiddenFileInput
            ...
            ref={fileInputRef}
          />
          <CustomButton onClick={fileClick}>写真を選択</CustomButton>
        </>
      ) : (
        <></>
      )}
      ...
      {setting ? (
        <Sinput
          ...
          onChange={(evt) => setUmail(evt.target.value)}
        ></Sinput>
      ) : (
        <Data>{umail}</Data>
      )}
      {setting ? (
        <SSideBarTextArea
          ...
          onChange={(evt) => setIntroductiontext(evt.target.value)}
        ></SSideBarTextArea>
      ) : introductiontext ? (
        <Intro>{getLines(introductiontext)}</Intro>
      ) : (
        <></>
      )}
      <br></br>
      {userInfo.id == userprofile.id ? (
        setting ? (
          <Sbutton onClick={Clicksetting}>編集を保存</Sbutton>
        ) : (
          <Sbutton
            onClick={() => {
              setSetting(true);
