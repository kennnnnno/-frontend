...
const relordclick = () => {
  setKeyword("");
    ...
    return(
        <SPostList>
            <p>
              <input placeholder="検索するキーワードを入力" value={keyword} onChange={(evt) => setKeyword(evt.target.value)}></input>
            ...
            <ButtonBox>
            {start!==0?<SnextbackButton onClick={()=>setStart(start-Listnum)}>＜＜前のページ</SnextbackButton>:<Span/>}
            {Math.floor(start / Listnum) + 1}/{totalPages}
  
            {Math.floor(start / Listnum) + 1<totalPages?<SnextbackButton onClick={()=>{setStart(start+Listnum)}}>次のページ＞＞</SnextbackButton>:<Span/>}
            </ButtonBox>
        </SPostList>
    )
}
