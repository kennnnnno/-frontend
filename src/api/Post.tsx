...
export const getList = async(token:string,start:number,records:number) => {
    const url = `${API_BASE_URL}/post?token=${token}&start=${start}&records=${records}`;
    const res = await axios.get(url);
    return res.data;
}

export const getUserList = async(token:string,start:number,records:number,id:number) => {
    const url = `${API_BASE_URL}/post/${id}?token=${token}&start=${start}&records=${records}`;
    const res = await axios.get(url);
    return res.data;
}

export const getSerchList = async(token:string,start:number,records:number,keyword:string) => {
